...
class BoomMenuButton @JvmOverloads constructor(
        ...
        if (boomButton.isNeededColorAnimation) {
            if (boomButton.prepareColorTransformAnimation()) {
                AnimationManager.animate(
                    ...
                    ShowRgbEvaluator.getInstance(), boomButton.pieceColor(), boomButton.buttonColor()
                )
            } else {
                AnimationManager.animate(
    ...
    private fun startEachReboomAnimation(
        ...
        if (boomButton.isNeededColorAnimation) {
            if (boomButton.prepareColorTransformAnimation()) {
                AnimationManager.animate(
                    boomButton, "rippleButtonColor", delay, duration,
                    HideRgbEvaluator.getInstance(), boomButton.buttonColor(), boomButton.pieceColor()
                )
            } else {
                AnimationManager.animate(
                    boomButton, "nonRippleButtonColor", delay, duration,
    ...
    private fun createButtons() {
        ...
        when (buttonEnum) {
            ButtonEnum.SimpleCircle -> {
                simpleCircleButtonRadius = (boomButtonBuilders[0] as SimpleCircleButton.Builder).buttonRadius
            }
            ButtonEnum.TextInsideCircle -> {
                textInsideCircleButtonRadius = (boomButtonBuilders[0] as TextInsideCircleButton.Builder).buttonRadius
            }
            ButtonEnum.TextOutsideCircle -> {
                textOutsideCircleButtonWidth = (boomButtonBuilders[0] as TextOutsideCircleButton.Builder).buttonContentWidth
                textOutsideCircleButtonHeight = (boomButtonBuilders[0] as TextOutsideCircleButton.Builder).buttonContentHeight
            }
            ButtonEnum.Ham -> {
                hamButtonWidth = (boomButtonBuilders[0] as HamButton.Builder).buttonWidth
                hamButtonHeight = (boomButtonBuilders[0] as HamButton.Builder).buttonHeight
            }
            ButtonEnum.Unknown -> { }
        }
    }
    ...
    override fun onButtonClick(index: Int, boomButton: BoomButton) {
        if (isAnimating()) return
        onBoomListener?.onClicked(index, boomButton)
